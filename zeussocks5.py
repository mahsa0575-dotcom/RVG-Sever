# zeussocks5.py
# ══════════════════════════════════════════════════════════════════════════════
# Zeus SOCKS5 — پروکسی SOCKS5 واقعی و مستقیم روی VPS
#   - پورت دلخواه (یا انتخاب خودکار از بازه‌ی تنظیمات)
#   - احراز هویت RFC1929 با نام کاربری/رمز تصادفی
#   - محدودیت حجم، انقضا و حداکثر اتصال همزمان per IP
#   - شمارش ترافیک و آمار اتصالات زنده در داشبورد
# ══════════════════════════════════════════════════════════════════════════════
...
def _pick_free_port(requested: Optional[int] = None) -> int:
    """پورت درخواستی اگر آزاد بود؛ وگرنه خطا. بدون درخواست، پورت موقت آزاد."""
    if requested:
        if not (1 <= requested <= 65535):
            raise RuntimeError("شماره پورت نامعتبر است")
        if not _port_free(requested):
            raise RuntimeError(f"پورت {requested} اشغال است")
        return requested
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(("0.0.0.0", 0))
...
async def _handle_client(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    if max_per_ip and _connections_by_ip.get(peer_ip, 0) >= max_per_ip:
        logger.info(f"ZeusSocks5: رد شد (حداکثر اتصال از {peer_ip}) ← {_connections_by_ip.get(peer_ip, 0)}/{max_per_ip}")
        writer.close()
        return
...
async def create_zeus_proxy(
    ...
    """ساخت پروکسی SOCKS5 روی پورت دلخواه VPS با کانفیگ حجم/انقضا/per-IP."""
    global _server

    if zeus_proxy_state.get("running") and zeus_proxy_state.get("phase") == "done":
        await delete_zeus_proxy()
    ...
    try:
        port = _pick_free_port(listen_port)
        ...
        logger.info(f"ZeusSocks5: سرور روی پورت {port} بالا آمد")
        ...
        config_str = f"{_creds['user']}:{_creds['password']}@{host}:{port}"
        result = {
            "user": _creds["user"],
...
async def delete_zeus_proxy():
    """سرور SOCKS5 را می‌بندد و state را پاک می‌کند."""
    global _server
