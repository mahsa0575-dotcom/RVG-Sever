...
# باگ داشتن، این ماژول مستقیماً باینری رسمی خودِ تلگرام
...
try:
    import resource  # فقط روی لینوکس (VPS) موجود است
except ImportError:
    resource = None
...
_bg_refs: set = set()  # رفرنس تسک‌های بک‌گراند تا GC نشوند
_instances: dict = {}
...
async def start_instance(
    ...
    async with _instances_lock:
        ...
        # ── چرا -6 ──
        # با -6 سوکت AF_INET6 با IPV6_V6ONLY=0 ساخته می‌شه یعنی dual-stack
        # (هم IPv4 هم IPv6). اگه محیطی IPv6 نداشت، بدون -6 fallback می‌کنیم.
        ...
        _bg_refs.add(asyncio.create_task(_watch_process(uuid, proc)))
        return inst
