...
def generate_ss_link(host: str, port: int, cipher: str, password: str, remark: str,
                     plugin: bool = True, ws_host: str | None = None) -> str:
    """لینک ss:// — با پلاگین v2ray-plugin (WS±TLS) یا کلاسیک بدون پلاگین (TCP بومی)."""
    import base64
    ...
    if plugin:
        tls_part = "tls;" if use_tls else ""
        plugin_str = f"v2ray-plugin;{tls_part}mux=0;path=/ss-ws;host={ws_host or host}"
        return f"ss://{userinfo}@{host}:{port}/?plugin={quote(plugin_str)}#{quote(remark)}"
