# central.py — ارتباط با سرویس مرکزی (اعلان‌ها و پشتیبانی)
# روی VPS این قابلیت به‌صورت پیش‌فرض «خاموش» است؛ فقط اگر CENTRAL_URL ست شده
# باشد پنل با سرور مرکزی شما تماس می‌گیرد (هیچ اطلاعات پیش‌فرضی جایی ارسال نمی‌شود).
import os
import asyncio
import httpx

CENTRAL_URL = os.environ.get("CENTRAL_URL", "").rstrip("/")
...
async def heartbeat_loop():
    if not CENTRAL_URL:
        # بدون سرور مرکزی، حلقه‌ی heartbeat لازم نیست
        return
    while True:
