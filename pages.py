# pages.py  -  RVG Gateway v10.0 (VPS Edition)
    ...
    <div>
      <section class="card">
        <div class="card-head stg">
          ...
          <span class="ver mono">v10.0</span>
        </div>
  ...
  <div class="modal-v2 cm-modal" style="max-width:500px">
    ...
    <div class="cm-body">
      ...
      <div id="zp-step-input">
        <div class="cm-section">
          <div class="cm-section-label"><i class="ti ti-plug-connected"></i> پورت و دسترسی</div>
          <div class="cm-field" style="margin-bottom:6px">
            <label>پورت SOCKS5</label>
            <input class="cm-input" id="zp-port" type="number" min="1" max="65535" placeholder="خالی = انتخاب خودکار">
            <div style="font-size:11px;color:var(--t3);margin-top:6px">این پورت مستقیماً روی سرور باز می‌شود؛ اگر فایروال دارید، آن را در پنل فایروال (ufw) باز کنید.</div>
          </div>
        </div>
      ...
      <div id="zp-step-building" style="display:none;text-align:center;padding:14px 0">
        ...
        <div style="color:var(--t3);font-size:12px;margin-top:4px">سرور SOCKS5 مستقیماً روی همین VPS بالا می‌آید</div>
      </div>
...
<section class="pg on" id="pg-overview">
  ...
  <div class="dash-footer">
    <span class="df-text">RVG Gateway v10.0 · VPS Edition</span>
  ...
  <div class="card" style="margin-bottom:16px">
    ...
    <div class="form-row" style="display:flex;gap:10px;flex-wrap:wrap">
      <div style="flex:2;min-width:220px">
        <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:6px">دامنه یا IP عمومی سرور (در لینک‌ها استفاده می‌شود)</label>
        <input class="cm-input" id="ss-host" placeholder="مثلاً vpn.example.com یا 5.6.7.8" style="direction:ltr;text-align:left">
      </div>
      <div style="flex:1;min-width:110px">
        <label style="font-size:11px;color:var(--t3);display:block;margin-bottom:6px">بازه پورت (از — تا)</label>
        <div style="display:flex;gap:6px">
          <input class="cm-input" id="ss-pr1" type="number" min="1" max="65535" placeholder="20000" style="direction:ltr;text-align:left">
          <input class="cm-input" id="ss-pr2" type="number" min="1" max="65535" placeholder="40000" style="direction:ltr;text-align:left">
        </div>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:10px;margin-top:12px;flex-wrap:wrap">
      <div class="sr" style="border:none;padding:0;gap:8px">
        <span class="sr-k" style="font-size:11.5px"><i class="ti ti-lock"></i> لینک‌ها با TLS (پشت nginx/CDN با گواهی)</span>
        <button class="tog" id="ss-tls-tog" onclick="toggleSsTls()"></button>
      </div>
      <button class="btn btn-p btn-sm" onclick="saveServerSettings()" style="margin-right:auto"><i class="ti ti-device-floppy"></i> ذخیره تنظیمات</button>
      <button class="btn btn-g btn-sm" onclick="resyncPorts()"><i class="ti ti-refresh"></i> بازسازی پورت‌ها</button>
    </div>
    <div id="ss-listeners" style="margin-top:12px;display:flex;flex-direction:column;gap:6px"></div>
    <div class="cl amber" style="margin-top:10px"><i class="ti ti-alert-triangle"></i><span>بعد از تغییر دامنه/TLS، لینک‌های ساخته‌شده قبلی خودکار به‌روز می‌شوند؛ اما کلاینت‌ها باید لینک جدید را دوباره ایمپورت کنند.</span></div>
  </div>
  <div class="g2">
    <div class="srv-panel">
      <div class="srv-hero">
        ...
        <div class="srv-hero-text">
          ...
          <div class="srv-hero-sub"><span class="dot dg pulse"></span> آنلاین · VPS اختصاصی</div>
        </div>
      </div>
      <div class="srv-tiles">
        <div class="srv-tile"><div class="srv-tile-icon"><i class="ti ti-route"></i></div><div class="srv-tile-text"><div class="srv-tile-label">پورت پنل</div><div class="srv-tile-val" id="srv-tile-port">—</div></div></div>
        ...
        <div class="srv-tile"><div class="srv-tile-icon"><i class="ti ti-server-bolt"></i></div><div class="srv-tile-text"><div class="srv-tile-label">پلتفرم</div><div class="srv-tile-val">VPS اختصاصی</div></div></div>
...
function protoBadge(p){
  const m={
    'shadowsocks':['Shadowsocks · WS','pc-ss'],
    'shadowsocks-tcp':['Shadowsocks · TCP','pc-ss'],
    'vless-ws':['VLESS · WS','pc-ws'],
    'vless-httpupgrade':['VLESS · HTTPUpgrade','pc-ws'],
    ...
    'trojan-httpupgrade':['Trojan · HTTPUpgrade','pc-trojan'],
    'trojan-xhttp-packet-up':['Trojan · XHTTP packet-up','pc-trojan'],
    'trojan-xhttp-stream-up':['Trojan · XHTTP stream-up','pc-trojan'],
    'vmess-ws':['VMess · WS','pc-trojan'],
    'vmess-httpupgrade':['VMess · HTTPUpgrade','pc-trojan'],
    'vmess-tcp':['VMess · TCP','pc-trojan'],
...
async function fetchStats(){
  try{
    ...
    document.getElementById('uptime-badge').textContent='VPS · '+d.uptime;
...
async function loadLinks(){
  try{
    const [lr,sr,nr,zr]=await Promise.all([authF('/api/links'),authF('/api/subs'),authF('/api/nodes/aggregate').catch(()=>null),authF('/api/zeus/status').catch(()=>null)]);
...
async function loadServerSettings(){
  try{
    const r = await authF('/api/settings/server'), d = await r.json();
    document.getElementById('ss-host').value = d.host || '';
    ssTls = !!d.tls;
    document.getElementById('ss-tls-tog').classList.toggle('on', ssTls);
    const pr = d.port_range || [20000, 40000];
    document.getElementById('ss-pr1').value = pr[0];
    document.getElementById('ss-pr2').value = pr[1];
    const tile = document.getElementById('srv-tile-port');
    if (tile) tile.textContent = toFa(d.panel_port ?? '—');
    await loadListeners();
  }catch(e){}
}
async function loadListeners(){
  try{
    const r = await authF('/api/ports'), d = await r.json();
    const box = document.getElementById('ss-listeners');
    const items = (d.listeners || []).map(L =>
      `<div class="sr" style="padding:7px 0;border:none">
        <span class="sr-k" style="font-size:11px"><i class="ti ${L.error?'ti-alert-triangle':'ti-circle-check'}" style="color:${L.error?'var(--red-t)':'var(--green-t)'}"></i>
        ${L.kind==='http'?'HTTP (ws/xhttp)':L.kind==='vmess'?'VMess · TCP':L.kind==='ss'?'Shadowsocks · TCP':L.kind==='mtproto'?'MTProto':L.kind}</span>
        <span class="sr-v" style="font-size:11px">${L.error ? esc(String(L.error)).slice(0,60) : 'پورت ' + toFa(L.port)}</span>
      </div>`).join('');
    box.innerHTML = items || '<div style="font-size:11px;color:var(--t3)">هنوز پورت اختصاصی‌ای باز نشده — همه‌ی کانفیگ‌ها روی پورت پنل سرو می‌شوند.</div>';
  }catch(e){}
}
async function saveServerSettings(){
  try{
    const body = {
      host: document.getElementById('ss-host').value.trim(),
      tls: ssTls,
      port_range: [Number(document.getElementById('ss-pr1').value||0), Number(document.getElementById('ss-pr2').value||0)],
    };
    const r = await authF('/api/settings/server', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
    const d = await r.json();
...
async function resyncPorts(){
  try{
    const r = await authF('/api/ports/resync', {method:'POST'});
    const d = await r.json();
    toast('پورت‌ها بازسازی شد ✓','ok');
    await loadListeners();
  }catch(e){ toast('✗ '+(e.message||'خطا'),'err'); }
}
...
let zeusStatus = null; // آخرین وضعیت دریافت‌شده از /api/zeus/status (توسط loadLinks پر می‌شود)
...
async function zpCheckTokenState(){
  // سازگاری اسم قدیمی — حالا فقط وضعیت ساخت را آماده می‌کند (بدون توکن)
  zpShowCreateStep('input');
  try{
    const r = await authF('/api/zeus/status'), d = await r.json();
    zeusStatus = d;
    const cfg = d.config || {};
...
async function zpStart(){
  const listen_port = document.getElementById('zp-port').value || null;
  ...
  try{
    const r = await authF('/api/zeus/create', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body: JSON.stringify({listen_port, traffic_limit_gb, expires_days, max_connections_per_ip})
    });
    const d = await r.json();
...
async function zpDelete(){
  if(!confirm('پروکسی Zeus حذف شود؟ سرور SOCKS5 بسته می‌شود.')) return;
  try{
    const r = await authF('/api/zeus/delete',{method:'POST'});
...
async function zpSaveConfig(){
  ...
  try{
    ...
    const rr = await authF('/api/zeus/status');
